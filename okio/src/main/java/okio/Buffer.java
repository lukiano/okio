...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable {
  ...
  @Override public long readHexadecimalUnsignedLong() throws IOException {
    if (size < 16) throw new IllegalStateException("size < 16: " + size);
    ...
    while (remaining > 0) {
      ...
      byte[] data = segment.data;
      int pos = segment.pos;
      int limit = Math.min(segment.limit, pos + remaining);
      int count = limit - pos;

      for (; pos < limit; pos++) {
        value <<= 4; // Shift for the new position.
        ...
        if (b >= '0' && b <= '9') {
          ...
          value |= b - 'A' + 10; // We never write uppercase, but we support reading it.
        } else {
          Buffer buffer = new Buffer().writeHexadecimalUnsignedLong(value >> 4); // Undo shift.
          buffer.skip(remaining - (pos - segment.pos)); // Skip unseen leading hex characters.
          throw new NumberFormatException(String.format("Non-hexadecimal byte 0x%x after '%s'.", b,
              buffer.readUtf8()));
        }
      }
  ...
  @Override public Buffer writeDecimalLong(long v) throws IOException {
    if (v == 0) {
    ...
    if (v < 100000000L) {
      if (v < 10000L) {
        ...
        if (v < 1000000L) {
          ...
          width = v < 10000000L ? 7 : 8;
        }
      }
    } else {
      if (v < 1000000000000L) {
        if (v < 10000000000L) {
          width = v < 1000000000L ? 9 : 10;
        } else {
  ...
  @Override public Buffer writeHexadecimalUnsignedLong(long v) throws IOException {
    Segment tail = writableSegment(16);
    byte[] data = tail.data;
    for (int pos = tail.limit + 15, start = tail.limit; pos >= start; pos--) {
